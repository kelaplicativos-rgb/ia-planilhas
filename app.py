...
APP_VERSION = "1.0.23"  # 🔥 versão incrementada
...
# =========================
    ...
    # 🔥 CORREÇÃO CRÍTICA: garantir DF antes de renderizar
    df_final = st.session_state.get("df_final")
    df_saida = st.session_state.get("df_saida")

    if df_final is None and df_saida is None:
        log_debug("Tentativa de acessar etapa FINAL sem dados", "ERROR")
        st.warning("⚠️ Nenhum dado disponível. Volte para o mapeamento.")
        st.button("⬅️ Voltar", on_click=lambda: _sincronizar_etapa_global("mapeamento"))
        st.stop()

    # 🔥 GARANTIA: sincronizar DF
