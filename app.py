...
from bling_app_zero.core.validator import (
    aplicar_validacao_gtin_df,
    formatar_numero_brasileiro,
    formatar_preview_valor,
    limpar_texto,
    normalizar_lista_urls_imagem,
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("♻️ Zerar mapeamento", width="stretch"):
    ...
    with st.expander("👀 Ver planilha do fornecedor", expanded=False):
        ...
        st.dataframe(df_origem_preview.head(20), width="stretch")
        ...
        st.dataframe(df_colunas, width="stretch", hide_index=True)
    ...
    with s1:
        if st.button("🧠 Recalcular sugestões", width="stretch"):
            mapeamento_auto, confianca = sugerir_mapeamento_local_refinado(df_origem, modo_operacao)
    ...
    with s2:
        ...
        if st.button("✨ Refinar com IA", width="stretch", disabled=not usar_openai):
    ...
    with sv1:
        if st.button("💾 Salvar mapeamento deste fornecedor", width="stretch"):
    ...
    if st.button("📦 Gerar planilha para conferência", width="stretch"):
        try:
