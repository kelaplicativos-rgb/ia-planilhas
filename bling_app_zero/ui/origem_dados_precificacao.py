...
from bling_app_zero.ui.origem_dados_estado import safe_str
...
def _render_header() -> None:
    st.markdown('<div class="op-kicker">Precificação</div>', unsafe_allow_html=True)
    st.markdown('<div class="op-title">Vai usar a calculadora de precificação?</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="op-sub">Você pode calcular automaticamente ou manter o preço da planilha fornecedora.</div>',
        unsafe_allow_html=True,
    )
...
def _persistir_resultado(df_resultado: pd.DataFrame) -> None:
    try:
        ...
        st.session_state["df_precificado"] = df_resultado

    try:
        st.session_state["df_calc_precificado"] = df_resultado.copy()
    except Exception:
        st.session_state["df_calc_precificado"] = df_resultado

    try:
        st.session_state["df_saida"] = df_resultado.copy()
    except Exception:
        st.session_state["df_saida"] = df_resultado
...
def _aplicar_precificacao_fluxo(df_origem: pd.DataFrame) -> None:
    ...
    coluna_base = safe_str(st.session_state.get("coluna_precificacao_resultado"))
    ...
    if isinstance(df_resultado, pd.DataFrame) and not df_resultado.empty:
        if safe_str(st.session_state.get("tipo_operacao_bling")).lower() == "estoque":
            ...
            df_resultado = aplicar_bloco_estoque(df_resultado, origem_atual)
...
def render_origem_precificacao() -> None:
    ...
    with st.expander("Abrir preview após precificação", expanded=False):
        df_preview = st.session_state.get("df_precificado") or st.session_state.get("df_calc_precificado")
