...
from bling_app_zero.core.global_dataset_guard import GLOBAL_FINAL_DATASET_SIGNATURE_KEY, dataframe_table_signature
...
from bling_app_zero.ui.flow_context import entry_context as _entry_context, is_bling_api_context as _legacy_is_api_context
...
RESPONSIBLE_FILE = 'bling_app_zero/ui/home_download_v2.py'
...
def df_signature(df: pd.DataFrame) -> str:
    return dataframe_table_signature(df, context='final_download')
...
def _normalized_columns(df: pd.DataFrame) -> list[str]:
    return [normalize_key(column) for column in df.columns] if isinstance(df, pd.DataFrame) else []
...
def _operation_contract_mismatch_error(raw_df: pd.DataFrame, download_df: pd.DataFrame, operation: str) -> str:
    ...
    if op == OP_ESTOQUE and _looks_like_cadastro_contract(download_df) and not _looks_like_stock_contract(download_df):
        add_audit_event('final_download_contract_mismatch_blocked', area='DOWNLOAD', status='BLOQUEADO', details={'operation': op, 'raw_columns': raw_columns, 'download_columns': download_columns, 'flow_spine': output_diagnostics(), 'responsible_file': RESPONSIBLE_FILE})
        return 'Operação bloqueada por segurança: você está em Atualização de estoque, mas a tabela final parece Cadastro. Volte e selecione a operação correta antes de baixar/enviar.'
    return ''
...
def _uploaded_model_contract_columns() -> list[str]:
    ...
    try:
        ...
        add_audit_event('uploaded_model_contract_read_failed', area='DOWNLOAD', status='IGNORADO', details={'template_name': template_name, 'error': str(exc), 'responsible_file': RESPONSIBLE_FILE})
        return []
...
def download_dataframe_for_contract(df: pd.DataFrame, operation: str) -> tuple[pd.DataFrame, bool, list[str]]:
    ...
    if uploaded_columns:
        ...
        add_audit_event('final_download_uses_uploaded_model_contract', area='DOWNLOAD', status='OK', details={'operation': normalize_operation(operation), 'columns': len(uploaded_columns), 'source': 'uploaded_model', 'responsible_file': RESPONSIBLE_FILE})
...
def _render_final_output_rule_report(report: FinalOutputRuleReport) -> bool:
    ...
    for warning in report.warnings:
        st.error(warning) if report.blocked_for_api else st.warning(warning)
...
def save_download_snapshot(*, download_df: pd.DataFrame, file_bytes: bytes, file_name: str, mime: str, operation: str, signature: str, rules_sig: str, widget_key: str) -> None:
...
def _render_direct_bling_send(download_df: pd.DataFrame, operation: str, key: str, signature: str, rules_sig: str) -> None:
    ...
    if not isinstance(targeted_df, pd.DataFrame) or targeted_df.empty:
        return
    target_signature = df_signature(targeted_df)
    st.session_state[FINAL_DOWNLOAD_SIGNATURE_KEY] = target_signature
    st.session_state[GLOBAL_FINAL_DATASET_SIGNATURE_KEY] = target_signature
    ...
    render_bling_api_batch_panel(targeted_df, operation, key, target_signature, rules_sig)
...
def render_download(df_final: pd.DataFrame, operation: str, key: str = 'final') -> None:
    ...
    if api_context:
        ...
        st.download_button(download_label(), data=file_bytes, file_name=file_name, mime=mime, use_container_width=True, key=f'download_model_backup_{key}_{signature}_{rules_sig}')
        return
...
__all__ = [
    'DESTINATION_MODEL_UPLOAD_OBJECT_KEY', 'DESTINATION_MODEL_UPLOAD_NAME_KEY', 'DESTINATION_MODEL_UPLOAD_BYTES_KEY',
    'FINAL_DOWNLOAD_DF_SNAPSHOT_KEY', 'FINAL_DOWNLOAD_FILE_BYTES_KEY', 'FINAL_DOWNLOAD_FILE_NAME_KEY', 'FINAL_DOWNLOAD_MIME_KEY',
    'FINAL_DOWNLOAD_SIGNATURE_KEY', 'FINAL_DOWNLOAD_RULES_SIGNATURE_KEY', 'FINAL_DOWNLOAD_OPERATION_KEY', 'FINAL_DOWNLOAD_WIDGET_KEY',
    'df_signature', 'download_final', 'download_label', 'render_download', 'save_download_snapshot', 'update_final_download_snapshot',
]
...
def update_final_download_snapshot(df: pd.DataFrame, filename: str, mime: str, file_bytes: bytes, *, operation: str | None = None) -> None:
    ...
    st.session_state[FINAL_DOWNLOAD_OPERATION_KEY] = normalize_operation(operation or '')
