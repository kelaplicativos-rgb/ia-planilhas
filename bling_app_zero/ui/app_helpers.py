...
def _campos_obrigatorios_por_operacao(df: pd.DataFrame) -> list[str]:
    """
    GTIN não entra como obrigatório no fluxo profissional/padrão Bling.
    """
    chaves_estado = [
        "campos_obrigatorios",
...
def _validar_campos_obrigatorios(df: pd.DataFrame) -> tuple[bool, list[str], list[str]]:
    """
    Regra profissional:
    - BLOQUEIA apenas quando a coluna obrigatória nem existe
      ou existe totalmente vazia.
    - Se a coluna existe e possui dados parciais, libera download
      e apenas alerta.
    """
    ...
    return len(faltantes_criticos) == 0, faltantes_criticos, alertas
...
def exportar_excel_bytes(df: pd.DataFrame) -> bytes:
    ...
    if _exportar_excel_robusto:
        try:
            resultado = _exportar_excel_robusto(df)
            if resultado:
                return resultado
        except Exception as e:
...
def render_preview_final() -> None:
    ...
    if not valido:
        ...
        st.error("⚠️ Existem campos obrigatórios críticos não preenchidos.")
        if texto_campos:
            st.caption(f"Campos críticos pendentes: {texto_campos}")

        log_debug(
            f"Campos obrigatórios críticos faltando: {texto_campos if texto_campos else 'não identificado'}",
            "ERROR",
        )
    else:
        log_debug("Validação final concluída sem campos obrigatórios críticos faltando.", "SUCCESS")

    if campos_alerta:
        st.warning(
            "Alguns campos obrigatórios têm linhas vazias, mas o arquivo pode ser baixado: "
            + ", ".join(campos_alerta)
        )
        log_debug(
