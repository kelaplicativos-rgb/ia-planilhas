...
def _render_botao_oauth_self(auth_url: str) -> None:
    ...
    st.markdown(
        f"""
        <a href="{auth_url_safe}" target="_self" style="
            display:block;
            width:100%;
            text-align:center;
...
def render_bling_primeiro_acesso(
    ...
    with col2:
        texto = "➡️ Continuar sem conectar"
        if conectado_real:
            texto = "➡️ Ir para origem"

        if st.button(
            texto,
            use_container_width=True,
            key="btn_continue_without_bling",
        ):
...
def render_send_panel() -> None:
    ...
    if conectado_real:
        ...
        st.warning(
            ...
            "O painel continua disponível, mas o envio seguirá somente como simulação até a conexão ser válida."
        )
