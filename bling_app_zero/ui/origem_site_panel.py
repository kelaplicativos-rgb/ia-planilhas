...
from bling_app_zero.core.instant_scraper.quality_score import (
    aplicar_score_qualidade,
    remover_produtos_fracos,
    resumo_qualidade,
)
...
def _aplicar_score_final(df: pd.DataFrame) -> pd.DataFrame:
    if not _df_ok(df):
        return pd.DataFrame()
    scored = aplicar_score_qualidade(df)
    return _ordenar_preview(scored)
...
def _salvar_df_origem(df: pd.DataFrame, origem: str) -> None:
    ...
    final = _aplicar_score_final(final)
    ...
    for candidato in candidatos:
        ...
        with st.container(border=True):
            ...
            if _df_ok(df_preview):
                preview = _aplicar_score_final(df_preview)
...
def _render_quality_summary(df: pd.DataFrame) -> None:
    ...
    c2.metric("Bons", resumo["bom"])
    c3.metric("Revisar", resumo["revisar"])
    c4.metric("Fracos", resumo["fraco"])

    if resumo["fraco"] > 0:
        if st.button("Remover produtos fracos", use_container_width=True):
            limpo = remover_produtos_fracos(df, score_minimo=50)
            limpo = _ordenar_preview(limpo)
            st.session_state["df_origem"] = limpo
            st.session_state["site_quality_cleaned"] = True
            st.success(f"Produtos fracos removidos. Restaram {len(limpo)} produto(s).")
...
def _render_resultado_final() -> None:
    ...
    if st.session_state.get("site_quality_cleaned") and _df_ok(st.session_state.get("df_origem")):
        base = st.session_state["df_origem"]
    else:
        base = _aplicar_score_final(df)
