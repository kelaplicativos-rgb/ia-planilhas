...
def _render_diagnostico_fetch() -> None:
    ...
    with st.expander("🔍 Diagnóstico da busca", expanded=False):
        ...
        st.write(f"Tamanho HTML: {info.get('html_chars', 0)}")
        ...
        if info.get("parece_javascript"):
            st.warning("Este site pode depender de JavaScript. O modo HTTP puro pode não capturar todos os produtos.")
...
def render_origem_site_panel() -> None:
    ...
    if limpar:
        _limpar_estado_busca_site()
        st.success("Busca limpa. Cache e estruturas anteriores foram removidos. Execute novamente.")
