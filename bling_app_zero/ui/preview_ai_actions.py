...
def _render_preview_ai_actions_inner(df_final: pd.DataFrame, operation: str) -> None:
    op = str(operation or 'arquivo').strip().lower() or 'arquivo'
    ...
    if ai_resource_ready():
        ...
        st.warning('Chave OpenAI não informada. A IA avançada fica indisponível, mas o fluxo manual continua normal.')

    _render_detected_columns(df_final)

    st.markdown('##### Ações da IA')
    ...
    with col_batch:
        batch_size = st.selectbox(
            ...
            help='Use 5 ou 10 quando houver timeout.',
        )
    with col_limit:
        max_rows_run = st.number_input(
            'Analisar nesta execução',
            min_value=5,
    ...
    run_label = f'🤖 Analisar produtos · {label}'
    if st.button(run_label, use_container_width=True, disabled=not can_run, key=_state_key(op, signature, 'run')):
        ...
        add_debug(f'IA executada de {current_offset + 1} até {result.next_offset} de {total_rows} linha(s) no preview final de {label}: {result.status}', origin='AI_RESOURCE_PLUGIN', level='INFO')
        st.success(f'IA analisou até {result.next_offset}/{total_rows} produto(s).')
        st.rerun()
    ...
    st.info(f'IA gerou {total_suggestions} sugestão(ões) para {affected_before_editor} produto(s). Produtos sem sugestão não serão alterados.')
    ...
    button_label = f'✅ Aplicar {apply_count} sugestão(ões) em {affected_products} produto(s)'
    if st.button(button_label, use_container_width=True, key=_state_key(op, signature, 'apply')):
        ...
        add_debug(f'{apply_count} sugestão(ões) de IA aplicadas em {affected_products} produto(s) na planilha final de {label}.', origin='AI_RESOURCE_PLUGIN', level='INFO')
...
def render_preview_ai_actions(df_final: pd.DataFrame | None, operation: str) -> None:
    ...
    with st.expander('🤖 Recursos de IA do preview · opcional', expanded=False):
        st.caption('Use apenas se quiser revisar textos, títulos, NCM ou sugestões antes do download. O preview e o download funcionam sem abrir esta área.')
        _render_preview_ai_actions_inner(df_final, operation)
