...
def _render_code_preview(title: str, code: str, key: str, *, language: str = 'javascript') -> None:
    """Renderiza código sem usar st.expander.

    Este helper pode ser chamado dentro de painéis que já estão em expander.
    O Streamlit não permite expander dentro de expander, então usamos checkbox.
    """
    show = st.checkbox(title, value=False, key=f'{key}_show_code')
    if show:
...
def _copy_split_bookmarklet(label: str, code: str, key: str) -> None:
    ...
    code_without_prefix = _without_javascript_prefix(code)

    _copy_button('1) Copiar somente javascript:', JAVASCRIPT_PREFIX, f'{key}_prefix')
    _copy_button('2) Copiar código SEM javascript:', code_without_prefix, f'{key}_without_prefix')

    _render_code_preview(
        'Ver código sem javascript:',
        code_without_prefix,
        f'{key}_without_prefix',
    )

    st.markdown('**Código completo com javascript:**')
    _copy_button('Copiar código completo', code, f'{key}_full')
    _render_code_preview(
        'Ver código completo com javascript:',
        code,
        f'{key}_full',
    )
