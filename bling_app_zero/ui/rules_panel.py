...
def _render_saved_column_rules(rules: dict) -> None:
    ...
    with st.expander(f'📌 Regras do usuário ({len(custom_rules)})', expanded=False):
        if not custom_rules:
            st.caption('Nenhuma regra criada ainda.')
            return

        st.caption('Essas regras preenchem automaticamente colunas do CSV final.')
...
def _render_resources_tab() -> None:
    ...
    st.caption('Recursos processam os dados automaticamente antes do download final.')
    ...
    updated['image_separator'] = st.text_input(
        ...
        help='Transforma múltiplas imagens para o padrão img1|img2|img3.',
    )
