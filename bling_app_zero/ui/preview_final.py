...
def _get_df_final_fluxo() -> pd.DataFrame | None:
    """
    Preview final endurecido:
    aceita apenas dataframes já transformados no fluxo.
    Não cai mais em df_origem para evitar exportação de base crua.
    """
    ...
    for chave in chaves_prioritarias:
        chave_limpa = _safe_str(chave)
        if not chave_limpa or chave_limpa in vistas:
            continue
        vistas.add(chave_limpa)

        df = _get_df_by_key(chave_limpa)
        if safe_df_dados(df):
            return df
...
def _get_deposito_nome() -> str:
    state = get_agent_state()
    return _safe_str(getattr(state, "deposito_nome", "") or st.session_state.get("deposito_nome"))
...
def _salvar_estado_final(df_blindado: pd.DataFrame, valido: bool, erros: list[str]) -> None:
    ...
    state.operacao = _get_tipo_operacao()
    state.deposito_nome = _get_deposito_nome()
...
def _render_resumo_agente_final() -> None:
    ...
    with st.expander("Estado do agente no preview final", expanded=False):
        ...
        if getattr(state, "erros", None):
            for erro in state.erros:
                st.error(erro)

        if getattr(state, "avisos", None):
...
def _render_tabela_principal(df_blindado: pd.DataFrame) -> None:
...
def render_preview_final() -> None:
    ...
    if not safe_df_dados(df_base):
        st.warning(
            "O ETL ainda não gerou uma saída final utilizável. "
            "Conclua normalização, mapeamento e montagem do modelo interno do Bling antes do download."
        )
        ...
        with col2:
            if st.button("↻ Recarregar preview final", use_container_width=True):
                st.rerun()

        render_debug_panel()
        log_debug("Preview final bloqueado: não há df final utilizável no fluxo.", "WARNING")
        return
    ...
    _render_tabela_principal(df_blindado)
