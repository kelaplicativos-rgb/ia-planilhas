...
def _current_flow() -> str:
    ...
    if stale_flow:
        add_audit_event(
            ...
            area='HOME',
            details={'reason': 'home_must_start_on_operation_choice', 'stale_flow': stale_flow, 'responsible_file': RESPONSIBLE_FILE},
        )
    return ''
...
def _render_operation_choice() -> None:
    ...
    _render_action_card(
        ...
        hint='Novo modo universal: anexe o modelo de destino e uma origem. A planilha final sai no mesmo formato do modelo anexado.',
        button_label='Começar pelo modelo de destino',
        button_key='home_open_universal_flow',
        on_click=_open_universal_flow,
    )
    st.caption('Fluxos operacionais mantidos')
    _render_action_card(
        icon='🧾',
        title='Cadastrar produtos',
        hint='Mantém busca por site, calculadora, mapeamento, preview e download final do fluxo de produtos.',
    ...
    _render_action_card(
        ...
        hint='Mantém o fluxo de estoque, incluindo origem por site/arquivo, depósito, mapeamento e download.',
    ...
    _render_action_card(
        ...
        hint='Mantém o fluxo específico para atualização de preços por loja/canal/marketplace.',
