...
def _extensao_permitida(nome: str, permitidas: list[str]) -> bool:
    nome = _safe_str(nome).lower()
    return any(nome.endswith(ext) for ext in permitidas)
...
def _render_upload_planilha_fornecedora() -> Optional[pd.DataFrame]:
    ...
    st.caption(
        "No celular, o seletor fica mais estável sem filtro rígido. "
        "Pode enviar CSV, XLSX ou XLS."
    )

    upload = st.file_uploader(
        "Toque para selecionar a planilha do fornecedor",
        key="origem_upload_fornecedor",
        help="Arquivos aceitos: .csv, .xlsx, .xls",
    )
    ...
    nome = _nome_upload(upload)
    if not _extensao_permitida(nome, [".csv", ".xlsx", ".xls"]):
        st.error("Arquivo inválido. Envie um CSV, XLSX ou XLS.")
        return None
    ...
    st.success(f"Planilha carregada: {upload.name}")
    return df_novo


def _render_upload_xml() -> Optional[pd.DataFrame]:
    st.markdown("#### Anexar XML da nota fiscal")

    upload_xml = st.file_uploader(
        "Toque para selecionar o XML",
        key="origem_upload_xml",
        help="Arquivo aceito: .xml",
    )

    if upload_xml is None:
        return None

    nome = _nome_upload(upload_xml)
    if not _extensao_permitida(nome, [".xml"]):
        st.error("Arquivo inválido. Envie um XML.")
        return None
    ...
    st.success(f"XML carregado: {upload_xml.name}")
    return df_novo


def _render_busca_site() -> Optional[pd.DataFrame]:
    st.markdown("#### Buscar pelo site")
...
def _render_fornecedor_api() -> Optional[pd.DataFrame]:
    st.markdown("#### Buscar por fornecedor via API")
...
def render_origem_dados() -> Optional[pd.DataFrame]:
    ...
    if (
        "df_modelo_operacao" not in st.session_state
        or st.session_state.get("df_modelo_operacao") is None
    ):
        st.session_state["df_modelo_operacao"] = _modelo_padrao_por_operacao(
    ...
    if origem_tipo == "Planilha fornecedora":
        df_novo = _render_upload_planilha_fornecedora()
    elif origem_tipo == "XML da nota fiscal":
        df_novo = _render_upload_xml()
    elif origem_tipo == "Buscar pelo site":
        df_novo = _render_busca_site()
    else:
        df_novo = _render_fornecedor_api()
