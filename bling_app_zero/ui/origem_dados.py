...
def _hash_arquivo_upload(arquivo) -> str:
    try:
        if arquivo is None:
            return ""
...
def _carregar_modelo_bling(arquivo, tipo_modelo: str) -> bool:
    ...
    try:
        hash_atual = _hash_arquivo_upload(arquivo)
        chave_hash = "modelo_cadastro_hash" if tipo_modelo == "cadastro" else "modelo_estoque_hash"
        ...
        if arquivo is not None:
            ...
            try:
                ...
                if _safe_df_dados(df_origem):
                    ...
                    st.session_state["arquivo_origem_nome"] = _nome_arquivo(arquivo)
                    log_debug(
...
def _validar_antes_mapeamento() -> tuple[bool, list[str]]:
    ...
    if not _safe_df_dados(st.session_st
