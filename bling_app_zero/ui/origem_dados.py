...
def _garantir_coluna(df: pd.DataFrame, coluna: str) -> pd.DataFrame:
    try:
...
def _aplicar_bloco_estoque(df_saida: pd.DataFrame, origem_atual: str) -> pd.DataFrame:
    try:
        ...
        if "site" in origem_atual:
            ...
            df_saida = _garantir_coluna(df_saida, "Quantidade")

            serie_qtd = df_saida["Quantidade"] if "Quantidade" in df_saida.columns else pd.Series(dtype="object")

            def _preencher_qtd(v):
                try:
            ...
            df_saida["Quantidade"] = serie_qtd.apply(_preencher_qtd)
