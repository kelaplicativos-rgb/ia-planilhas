...
def render_shared_calculator(
    ...
    *,
    ...
    with st.expander('Aplicar cálculo marketplace antes do mapeamento', expanded=expanded):
        ...
        with st.expander('Avançado: nome técnico da coluna calculada na origem', expanded=False):
            custom_output = st.text_input(
                'Nome técnico usado no mapeamento',
                value=str(output_column or 'Preço calculado marketplace'),
                key=f'{key_prefix}_price_output_name_advanced',
                help='Normalmente deixe igual à coluna do modelo. Só altere se quiser criar uma coluna auxiliar diferente.',
            ).strip()
            if custom_output:
