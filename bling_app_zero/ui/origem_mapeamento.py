...
def _is_coluna_id(nome) -> bool:
    nome = str(nome).lower().strip()
    return "id produto" in nome or nome == "id"
...
def render_origem_mapeamento():
    ...
    for col_modelo in df_modelo.columns:
        ...
        # 🔥 remove já usadas
        opcoes = [""] + [c for c in df_origem.columns if c not in usadas]
    ...
    if duplicidades:
        erro = True
        st.error("❌ Existe coluna sendo usada mais de uma vez. Corrija antes de continuar.")
