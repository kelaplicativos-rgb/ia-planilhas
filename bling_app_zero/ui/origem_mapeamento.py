...
def _normalizar_chave(valor) -> str:
    try:
        texto = _normalizar_texto_coluna(valor).lower()
        substituicoes = {
            "á": "a",
            "à": "a",
            "ã": "a",
...
# =========================
# AUTO SUGESTÃO / FALLBACK
# =========================
def _mapa_colunas_normalizadas(df: pd.DataFrame) -> dict[str, str]:
    mapa: dict[str, str] = {}
    try:
        for col in df.columns:
            chave = _normalizar_chave(col)
...
def _eh_coluna_codigo(nome: str) -> bool:
    n = _normalizar_chave(nome)
    palavras = ["codigo", "sku", "referencia", "ref", "id produto", "cod produto"]
    return any(p in n for p in palavras)


def _eh_coluna_nome(nome: str) -> bool:
    n = _normalizar_chave(nome)
    palavras = ["nome", "produto", "titulo", "descricao", "descrição"]
    return any(p in n for p in palavras)


def _eh_coluna_preco(nome: str) -> bool:
    n = _normalizar_chave(nome)
    palavras = ["preco", "valor", "venda", "preco venda", "valor venda"]
    return any(p in n for p in palavras)


def _eh_coluna_estoque(nome: str) -> bool:
    n = _normalizar_chave(nome)
    palavras = ["estoque", "saldo", "quantidade", "qtd", "qtde"]
    return any(p in n for p in palavras)


def _sugerir_coluna_origem(col_modelo: str, df_origem: pd.DataFrame) -> str:
    try:
        if not _safe_df(df_origem):
            return ""

        modelo_norm = _normalizar_chave(col_modelo)
        if not modelo_norm:
            return ""

        colunas_origem = list(df_origem.columns)
        mapa_norm = _mapa_colunas_normalizadas(df_origem)

        # 1) mesmo nome normalizado
        if modelo_norm in mapa_norm:
            return mapa_norm[modelo_norm]

        # 2) contém direto
        for col in colunas_origem:
            origem_norm = _normalizar_chave(col)
            if not origem_norm:
                continue
            if modelo_norm == origem_norm:
                return col
            if modelo_norm in origem_norm or origem_norm in modelo_norm:
                return col

        # 3) heurísticas por grupo
        if _eh_coluna_codigo(col_modelo):
            for col in colunas_origem:
                if _eh_coluna_codigo(col):
                    return col

        if _eh_coluna_nome(col_modelo):
            for col in colunas_origem:
                if _eh_coluna_nome(col):
                    return col

        if _eh_coluna_preco(col_modelo):
            for col in colunas_origem:
                if _eh_coluna_preco(col):
                    return col

        if _eh_coluna_estoque(col_modelo):
            for col in colunas_origem:
...
def _resolver_mapping_inicial(df_modelo: pd.DataFrame, df_origem: pd.DataFrame) -> dict:
    mapping_salvo = st.session_state.get("mapping_origem", {})
    if not isinstance(mapping_salvo, dict):
        mapping_salvo = {}

    mapping_inicial: dict[str, str] = {}

    for col_modelo in df_modelo.columns:
        if _is_coluna_deposito(col_modelo):
            continue

        valor_salvo = str(mapping_salvo.get(col_modelo, "") or "").strip()
        if valor_salvo and valor_salvo in df_origem.columns:
            mapping_inicial[col_modelo] = valor_salvo
            continue

        sugestao = _sugerir_coluna_origem(col_modelo, df_origem)
        mapping_inicial[col_modelo] = sugestao if sugestao in df_origem.columns else ""

    return mapping_inicial
...
def _montar_df_saida(df_origem, df_modelo, mapping):
    ...
    for col in df_modelo.columns:
        ...
        if origem and origem in df_base.columns:
            df_saida[col] = df_base[origem].reset_index(drop=True)
            continue
        ...
        # 2) fallback pelo mesmo nome da coluna do modelo
        if col in df_base.columns:
            df_saida[col] = df_base[col].reset_index(drop=True)
            continue

        if col in df_origem.columns:
            df_saida[col] = df_origem[col].reset_index(drop=True)
            continue

        # 3) fallback por nome semelhante
        sugestao = _sugerir_coluna_origem(col, df_base)
        if sugestao and sugestao in df_base.columns:
            df_saida[col] = df_base[sugestao].reset_index(drop=True)
            continue

        sugestao = _sugerir_coluna_origem(col, df_origem)
        if sugestao and sugestao in df_origem.columns:
            df_saida[col] = df_origem[sugestao].reset_index(drop=True)
            continue
...
def render_origem_mapeamento():
    ...
    mapping_inicial = _resolver_mapping_inicial(df_modelo, df_origem)
    ...
    for col_modelo in df_modelo.columns:
        ...
        valor_atual = st.session_state.get(
            f"map_{col_modelo}",
            mapping_inicial.get(col_modelo, ""),
        )
    ...
    if duplicidades:
        ...
        for origem, destinos in duplicidades.items():
            textos.append(f"{origem} → {', '.join(destinos)}")
        st.warning(
            "Atenção: há colunas da origem sendo reutilizadas em mais de um campo do modelo. "
            + " | ".join(textos)
        )
