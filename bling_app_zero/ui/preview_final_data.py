...
from bling_app_zero.core.bling_categories import coluna_categoria_bling_existente, normalizar_categoria_bling
...
def garantir_coluna_categoria_sistema(df: pd.DataFrame, tipo_operacao: str) -> pd.DataFrame:
    if not isinstance(df, pd.DataFrame):
        return pd.DataFrame()
    base = df.copy().fillna("")
    if normalizar_texto(tipo_operacao) == "estoque":
        return base.fillna("")

    coluna_origem = coluna_categoria_bling_existente(base.columns)
    if "Categoria do sistema" not in base.columns:
        base["Categoria do sistema"] = ""

    if coluna_origem and coluna_origem in base.columns:
        base["Categoria do sistema"] = base[coluna_origem].apply(normalizar_categoria_bling)
        log_debug(f"Categoria do sistema normalizada para padrão Bling a partir de: {coluna_origem}", nivel="INFO")
    else:
        base["Categoria do sistema"] = base["Categoria do sistema"].apply(normalizar_categoria_bling)
...
def garantir_df_final_canonico(df: pd.DataFrame, tipo_operacao: str, deposito_nome: str) -> pd.DataFrame:
    ...
    base = garantir_coluna_categoria_sistema(base, tipo_operacao)
