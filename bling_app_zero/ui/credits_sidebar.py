...
def render_credits_sidebar() -> None:
    with st.sidebar:
        with st.expander('💳 Créditos MapeiaAI', expanded=False):
            st.caption(f'Modelo preparado: {credit_price_label()}. Pagamento real ainda não está conectado.')
            ...
            with st.expander('Pacotes planejados', expanded=False):
            ...
            with col_b:
                if st.button('+20 créditos', use_container_width=True, key='mapeiaai_add_20_demo_credits'):
                    add_demo_credits(20)
                    st.rerun()
            st.caption(f'Produção: conectar checkout em {APP_PUBLIC_DOMAIN} e gravar créditos em banco por usuário.')
