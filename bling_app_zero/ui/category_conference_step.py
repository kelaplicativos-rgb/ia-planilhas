...
RESPONSIBLE_FILE = 'bling_app_zero/ui/category_conference_step.py'
...
def _df_signature(df: pd.DataFrame) -> str:
    ...
    sample = pd.util.hash_pandas_object(df.head(300).fillna('').astype(str), index=True).sum()
...
def _reset_if_source_changed(df: pd.DataFrame, source_key: str) -> None:
    ...
    if previous and previous != signature:
        ...
        add_audit_event(
            ...
            status='OK',
            details={'previous_signature': previous, 'current_signature': signature, 'cache_invalidated': True, 'responsible_file': RESPONSIBLE_FILE},
        )
...
def render_category_conference_step() -> None:
    ...
    st.warning('Proteção anti-cache: esta etapa usa a origem viva atual. Se a base mudar, a confirmação antiga é invalidada automaticamente.')
    ...
    with c2:
        if st.button('⏭️ Pular sem alterar categorias', use_container_width=True, key='category_conference_skip_v1'):
            ...
            st.rerun()
    with c3:
        st.download_button('⬇️ Baixar relatório da conferência', data=_csv_bytes(analyzed), file_name='conferencia_inteligente_categorias.csv', mime='text/csv', use_container_width=True, key='category_conference_download_v1')
