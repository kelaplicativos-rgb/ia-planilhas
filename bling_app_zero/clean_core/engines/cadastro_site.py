...
from .site_tools import extract_gtin, extract_images, extract_price, extract_stock, extract_title, fetch_url, should_extract


class CadastroSiteEngine:
    operation = Operation.CADASTRO
    name = "Motor independente: cadastro por site"
    ...
    def run(self, *, model_df: pd.DataFrame, requested_schema: list[RequestedField], urls: list[str], deposit_name: str = "") -> EngineResult:
        ...
        for url in clean_urls:
            ...
            if should_extract(FieldIntent.PRECO, requested_schema):
                payload[FieldIntent.PRECO] = extract_price(snapshot)

            if should_extract(FieldIntent.IMAGEM, requested_schema):
                payload[FieldIntent.IMAGEM] = extract_images(snapshot)

            if should_extract(FieldIntent.GTIN, requested_schema):
                payload[FieldIntent.GTIN] = extract_gtin(snapshot)
            ...
            if should_extract(FieldIntent.FORNECEDOR, requested_schema):
                payload[FieldIntent.FORNECEDOR] = "Não definido"

            payloads.append(payload)
            logs.append(f"Cadastro: capturado somente o solicitado pelo modelo em {snapshot.url}")
