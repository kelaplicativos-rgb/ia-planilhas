...
def _sleep_rate_limit(min_interval_seconds: float = 0.38) -> None:
    st = _streamlit_ctx()
    if st is None:
        time.sleep(min_interval_seconds)
        return

    ultimo = float(st.session_state.get("_bling_last_request_at", 0.0) or 0.0)
    agora = time.time()
    decorrido = agora - ultimo

    if decorrido < min_interval_seconds:
        time.sleep(min_interval_seconds - decorrido)

    st.session_state["_bling_last_request_at"] = time.time()
    ...
    if bling_auth is not None:
        ...
        if manager_cls is not None:
            try:
                ...
                if hasattr(auth, "store") and hasattr(auth.store, "get"):
                    token_payload = auth.store.get(user_key)
                    token = _normalizar_texto(token_payload.get("access_token"))
                    if token:
                        return token
...
class BlingApiClient:
    def __init__(self, access_token: str, timeout: int = 30, max_retries: int = 4) -> None:
        ...
        *,
        ...
        for tentativa in range(1, self.max_retries + 1):
            try:
                ...
                if response.status_code in {429, 500, 502, 503, 504} and tentativa < self.max_retries:
                    espera = min(2.5, 0.6 * tentativa)
                    _log_debug(
                        f"Retry Bling | tentativa={tentativa} | status={response.status_code} | endpoint={endpoint}",
                        nivel="INFO",
                    )
                    time.sleep(espera)
                    continue
                ...
                if tentativa < self.max_retries:
                    espera = min(2.5, 0.6 * tentativa)
                    _log_debug(
                        f"Falha de request Bling | tentativa={tentativa} | endpoint={endpoint} | erro={exc}",
                        nivel="ERRO",
                    )
                    time.sleep(espera)
                    continue
        ...
        raise RuntimeError(str(ultima_excecao or "Falha desconhecida na requisição ao Bling."))
...
def _emitir_progresso(
    ...
    *,
    ...
    if not force and progress_update_every > 1 and processados % progress_update_every != 0:
        return

    payload = {
        "processados": processados,
        "total": total,
        "codigo": item_resultado.get("codigo"),
        "descricao": item_resultado.get("descricao"),
        "status": item_resultado.get("status"),
        "mensagem": item_resultado.get("mensagem"),
        "acao": item_resultado.get("acao"),
        "linha": item_resultado.get("linha"),
    ...
    try:
        ...
        _log_debug(f"Falha no callback de progresso: {exc}", nivel="ERRO")
...
def sincronizar_produtos_bling(
    ...
    config = SyncConfig(
        ...
        progress_update_every=max(1, int(progress_update_every or 1)),
    )
    ...
    _log_debug(
        ...
        f"progress_update_every={config.progress_update_every}",
        nivel="INFO",
    )
    ...
    for indice, row in df.fillna("").iterrows():
        ...
        if not codigo:
            ...
            _emitir_progresso(
                ...
                total_ignorados=total_ignorados,
                total_erros=total_erros,
                progress_update_every=config.progress_update_every,
            )
            continue
