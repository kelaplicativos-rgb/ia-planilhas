...
def _emit_status(status_callback: Callable[[dict[str, Any]], None] | None, payload: dict[str, Any]) -> None:
    if callable(status_callback):
        try:
            status_callback(payload)
        except Exception as exc:
            _log_debug(f"Falha ao emitir status visual do envio: {exc}", nivel="ERRO")


def _coluna_existente(df: pd.DataFrame, nomes: list[str]) -> str:
    mapa = {_safe_lower(c): str(c) for c in df.columns}
    for nome in nomes:
        achado = mapa.get(_safe_lower(nome))
        if achado:
            return achado
    return ""
...
def _coluna_codigo(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["Código", "codigo", "Código do produto", "SKU", "sku"]) or _coluna_por_parcial(
        df,
...
def _coluna_descricao(df: pd.DataFrame) -> str:
    ...
    ) or _coluna_por_parcial(df, ["descricao", "descrição", "nome", "titulo", "título", "produto"])


def _coluna_descricao_curta(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["Descrição Curta", "descricao curta", "Descrição curta"]) or _coluna_por_parcial(
        df,
        ["descricao curta", "descrição curta", "resumo"],
    )


def _coluna_preco_venda(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["Preço de venda", "preço de venda", "Preço calculado"]) or _coluna_por_parcial(
        df,
        ["preco", "preço", "valor", "venda"],
    )
...
def _coluna_preco_estoque(df: pd.DataFrame) -> str:
    return _coluna_existente(
        df,
        ["Preço unitário (OBRIGATÓRIO)", "preço unitário (obrigatório)"],
    ) or _coluna_por_parcial(
        df,
        ["preco unitario", "preço unitário", "preco", "preço", "valor unitario", "valor unitário"],
    )


def _coluna_gtin(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["GTIN/EAN", "GTIN", "EAN", "gtin", "ean"]) or _coluna_por_parcial(
        df,
...
def _coluna_categoria(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["Categoria", "categoria"]) or _coluna_por_parcial(
        df,
...
def _coluna_situacao(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["Situação", "situacao"]) or _coluna_por_parcial(
        df,
...
def _coluna_estoque(df: pd.DataFrame) -> str:
    ...
    ) or _coluna_por_parcial(df, ["balanco", "balanço", "quantidade", "estoque", "saldo", "qtd"])


def _coluna_deposito(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["Depósito (OBRIGATÓRIO)", "depósito (obrigatório)", "Deposito"]) or _coluna_por_parcial(
        df,
        ["deposito", "depósito", "almoxarifado"],
    )


def _coluna_ncm(df: pd.DataFrame) -> str:
    return _coluna_existente(df, ["NCM", "ncm"]) or _coluna_por_parcial(df, ["ncm"])
...
def _obter_access_token() -> str:
    ...
    try:
        ...
        if ok:
            ...
            if token:
                return token
        _log_debug(f"Token do Bling indisponível: {token_or_msg}", nivel="ERRO")
    except Exception as exc:
        _log_debug(f"Falha ao obter token do Bling: {exc}", nivel="ERRO")
...
class BlingApiClient:
    ...
    def _throttle(self) -> None:
        ...
        wait_for = API_MIN_INTERVAL_SECONDS - elapsed
        if wait_for > 0:
            time.sleep(wait_for)
    ...
    def _request(
        ...
        *,
        ...
        if not self.disponivel:
            raise RuntimeError("Token do Bling não encontrado ou expirado.")

        self._throttle()
        url = f"{BLING_API_BASE.rstrip('/')}/{endpoint.lstrip('/')}"

        payload = {"data": json_payload} if json_payload is not None else None
        ...
        return conteudo if isinstance(conteudo, dict) else {"data": conteudo}
    ...
    def buscar_produto_por_codigo(self, codigo: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            _log_debug(f"Falha ao buscar produto por código {codigo}: {exc}", nivel="ERRO")
            return None

        for item in resposta.get("data", []) or []:
            if _normalizar_texto(item.get("codigo")) == codigo:
                return item
...
def _montar_payload_produto(row: pd.Series, df: pd.DataFrame, config: SyncConfig) -> dict[str, Any]:
    ...
    preco = _to_float(
        row.get(preco_estoque_col) if config.tipo_operacao == "estoque" else row.get(preco_venda_col),
        0.0,
    )
...
def _proxima_execucao(auto_mode: str, interval_value: int, interval_unit: str) -> str:
    if _safe_lower(auto_mode) != "periodico":
        return ""
...
def _obter_metadata_site() -> dict[str, Any]:
    st = _streamlit_ctx()
    if st is None:
        return {"origem_site_ativa": False, "url_site": ""}
...
def _registrar_agendamento_local(config: SyncConfig, total_itens: int) -> dict[str, Any]:
    info = {
        ...
        "proxima_execucao": _proxima_execucao(config.auto_mode, config.interval_value, config.interval_unit),
        "observacao": (
            "Configuração registrada na aplicação. Rotinas recorrentes dependem de execução contínua "
            "na infraestrutura do app."
        ),
...
def _executar_fallback_site_se_necessario(config: SyncConfig) -> dict[str, Any]:
    ...
    if config.auto_mode != "instantaneo":
        resultado["observacao"] = "Origem por site detectada. Fallback instantâneo não foi solicitado."
        return resultado
    ...
    if not url_site or buscar_produtos_site_com_gpt is None:
        resultado["observacao"] = "Modo instantâneo solicitado, mas a URL ou o módulo de busca por site não está disponível."
        return resultado

    try:
        df_site = buscar_produtos_site_com_gpt(base_url=url_site, diagnostico=True)
        ...
        resultado["observacao"] = "Busca por site executada no modo instantâneo antes do envio."
    except Exception as exc:
...
def sincronizar_produtos_bling(
    ...
    if not _safe_df_dados(df):
        ...
        _emit_status(status_callback, {"phase": "finish", "summary": resumo})
        return resumo
    ...
    if not codigo_col or not descricao_col:
        mensagem = (
            "Coluna de código não encontrada na planilha final."
            if not codigo_col
            else "Coluna de descrição não encontrada na planilha final."
        )
        resumo = {
            "ok": False,
            "modo": "validacao",
            ...
            "resultados": [],
            "fallback_site": _executar_fallback_site_se_necessario(config),
            "agendamento": _registrar_agendamento_local(config, total_itens),
        }
        _persistir_resultado_sync(resumo)
        _emit_status(status_callback, {"phase": "finish", "summary": resumo})
        return resumo
    ...
    for sequencia, (indice, row) in enumerate(df.fillna("").iterrows(), start=1):
        codigo = _normalizar_texto(row.get(codigo_col))
        descricao = _normalizar_texto(row.get(descricao_col))
        ...
        try:
            ...
            if client.disponivel and not config.dry_run and config.strategy != "cadastrar_novos":
            ...
            if acao == "criar":
                ...
                if not produto_id:
                    raise RuntimeError("Produto existente não encontrado para atualização.")
    ...
    resumo = {
        "ok": ok,
        "modo": modo,
        "mensagem": "Sincronização concluída com sucesso." if ok else "Sincronização concluída com pendências/erros.",
        "processado_em": _agora_iso(),
        "tipo_operacao": config.tipo_operacao,
        "strategy": config.strategy,
        "auto_mode": config.auto_mode,
