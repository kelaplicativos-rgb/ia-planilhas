...
def run_stable_app() -> None:
    ...
    with tab_arquivo:
        uploaded = st.file_uploader(
            "Anexar arquivo do fornecedor",
            type=None,
            key="stable_upload_origem",
            help="Modo livre para Android. Aceita CSV, Excel, TXT, HTML, JSON e similares.",
        )
        if uploaded is not None:
            try:
                df_origem = _read_upload(uploaded)
                st.session_state["stable_df_origem"] = df_origem
                st.success(f"Arquivo lido: {len(df_origem)} linhas × {len(df_origem.columns)} colunas")
