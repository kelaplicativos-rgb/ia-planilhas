    ...
    try:
        ...
        def after_write_with_image_client(self, product_id: str, payload: dict, attempts: list):
            ...
            if _has_images(saved):
                return saved
            ok, image_attempts = push_product_images(
            ...
            add_audit_event(
                'bling_image_runtime_after_write_finished',
                area='BLING_IMAGEM',
                status='OK' if ok or _has_images(refreshed) else 'AVISO',
                details={
                    'product_id': str(product_id),
                    'image_client_ok': ok,
                    'has_images_after': _has_images(refreshed),
                    'attempts': image_attempts[-20:],
        ...
        _INSTALLED = True
        add_audit_event(
            'bling_image_runtime_installed',
            area='BLING_IMAGEM',
            status='OK',
            details={'responsible_file': RESPONSIBLE_FILE, 'client_file': 'bling_app_zero/core/bling_product_image_client.py'},
        )
        return True
    except Exception as exc:
        _INSTALLED = True
        add_audit_event(
            'bling_image_runtime_install_failed',
            area='BLING_IMAGEM',
            status='AVISO',
