...
def _classify_content(
    *,
    ...
    # Tipos com assinatura forte primeiro. Isso impede que um cabeçalho "Produto"
    # com EANs seja tratado como descrição só pelo nome da coluna.
    if gtin >= 0.55:
    ...
    # Números inteiros curtos, sem centavos, são estoque/quantidade com mais chance
    # do que preço quando o cabeçalho não indica claramente preço.
    ...
    if text >= 0.45 and avg_len <= 35 and numeric < 0.20 and url < 0.10:
        if unique <= 0.75 or header_kind == 'marca':
            return 'marca', min(0.90, 0.50 + text * 0.25 + (0.10 if unique <= 0.75 else 0)), 'As amostras são textos curtos recorrentes, compatíveis com marca/fabricante.'

    if numeric >= 0.65:
        return 'codigo', min(0.86, 0.45 + numeric * 0.30), 'As amostras são majoritariamente numéricas, mas sem assinatura forte de preço, GTIN ou estoque.'
...
def analyze_column_semantics(df: Any, column: str, *, limit: int = 120) -> SemanticColumnProfile:
    ...
    gtin = _ratio(sum(1 for value in values if GTIN_RE.match(_compact_number(value))), total)
    ...
    content_kind, confidence, reason = _classify_content(
    ...
    header_conflict = bool(
        header_kind != 'custom'
        and content_kind != 'custom'
        and not _kinds_compatible(header_kind, content_kind)
        and confidence >= 0.55
    )
    effective_kind = content_kind if content_kind != 'custom' and confidence >= 0.50 else header_kind
    warning = ''
    if header_conflict:
        warning = f'Cabeçalho sugere {header_kind}, mas as linhas parecem {content_kind}. Revisar antes de aceitar automático.'
...
def profile_as_mapping(df: Any, column: str, *, limit: int = 120) -> dict[str, Any]:
    ...
    data = asdict(profile)
    # Compatibilidade com o contrato antigo: quem lia "kind" agora recebe o tipo
    # efetivo validado por conteúdo, não apenas o tipo inferido pelo cabeçalho.
