"""
FORNECEDOR ATACADUM (SCRAPER DEDICADO PRO)

Padrão:
- Sitemap FIRST
- Estoque PRIORIDADE TOTAL
- Entrada em detalhe obrigatória
"""
...
class AtacadumSupplier(SupplierBase):

    nome = "Atacadum"
    dominio = ["atacadum.com.br"]

    # -------------------------------
    # FETCH PRINCIPAL
    # -------------------------------
    def fetch(self, url: str, limite: int = 300, **kwargs) -> List[Dict]:
        ...
        # ===============================
        if not produtos:
            produtos.extend(self._fallback_lista(url))
    ...
    # NOME
    # -------------------------------
    def _extrair_nome(self, soup, fallback: str) -> str:
    ...
    # IMAGENS
    # -------------------------------
    def _extrair_imagens(self, soup, base_url: str):
    ...
    # FALLBACK LISTA
    # -------------------------------
    def _fallback_lista(self, url: str) -> List[Dict]:
