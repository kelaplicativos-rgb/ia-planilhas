...
def _target_kind(target: str) -> str:
    ...
    for kind_name, aliases in FIELD_ALIASES.items():
        if any(normalize_key(alias) in target_key for alias in aliases):
            return kind_name
    return 'custom'
...
def _is_blocked_target(target: str) -> bool:
    ...
    return any(term in key for term in BLOCK_TARGET_TERMS)
...
def _name_score(target: str, source: str, target_kind: str) -> int:
    ...
    for alias in FIELD_ALIASES.get(target_kind, []):
        ...
        if alias_key and alias_key in source_key:
            score += 55
...
def _content_score(target_kind: str, profile: dict[str, float | str]) -> int:
...
def _best_candidate(df: pd.DataFrame, target: str, source_columns: list[str], used: set[str]) -> Candidate:
    ...
    if _is_blocked_target(target) and target_kind == 'custom':
        return Candidate('', 0)
    ...
    for source in source_columns:
        ...
        score = _name_score(target, source, target_kind) + _content_score(target_kind, profile)
        if score > best.score:
...
def super_auto_map_columns(df_source: pd.DataFrame, df_model: pd.DataFrame, min_score: int = 80) -> dict[str, str]:
    ...
    used = {source for source in base_mapping.values() if source}
    mapping = dict(base_mapping)

    for target in [str(c) for c in df_model.columns]:
