...
def _patch_complete_product_update() -> bool:
    ...
    try:
        ...
        def update_existing_product_if_changed_v3(*, token, product_id, variants, url_builder, headers_builder, timeout, responsible_file):
            ...
            if first_payload is not None and _has_not_found(attempts):
                ...
                for attempt in create_attempts:
                    ...
                    attempts.append(item)
                if created_id:
                    add_audit_event('bling_v3_created_after_stale_deleted_id', area='BLING_ENVIO', status='OK', details={'old_product_id': str(product_id), 'new_product_id': str(created_id), 'attempts': attempts[-8:], 'responsible_file': RESPONSIBLE_FILE})
                    return 'updated', attempts
            add_audit_event('bling_v3_update_existing_failed_no_create_condition', area='BLING_ENVIO', status='AVISO', details={'product_id': str(product_id), 'statuses': _attempt_statuses(attempts), 'has_not_found': _has_not_found(attempts), 'attempts': attempts[-8:], 'responsible_file': RESPONSIBLE_FILE})
            return 'failed', attempts

        original = getattr(bling_smart_product_diff, '_blingfix_original_update_existing_product_if_changed', None)
        if original is None:
            setattr(bling_smart_product_diff, '_blingfix_original_update_existing_product_if_changed', bling_smart_product_diff.update_existing_product_if_changed)
        ...
        add_audit_event('bling_complete_product_update_patch_failed', area='BLING_ENVIO', status='AVISO', details={'error': str(exc)[:220], 'responsible_file': RESPONSIBLE_FILE})
...
def _install_review() -> bool:
    try:
        ...
        add_audit_event('bling_review_install_call_failed', area='BLING_ENVIO', status='AVISO', details={'error': str(exc)[:220], 'responsible_file': RESPONSIBLE_FILE})
        return False
...
def _install_image_client() -> bool:
    try:
        ...
        add_audit_event('bling_image_runtime_call_failed', area='BLING_IMAGEM', status='AVISO', details={'error': str(exc)[:220], 'responsible_file': RESPONSIBLE_FILE})
        return False


def _install_verified_sender() -> bool:
    try:
        from bling_app_zero.core.verified_sender_runtime import install_verified_sender_runtime
        return bool(install_verified_sender_runtime())
    except Exception as exc:
        add_audit_event('verified_sender_runtime_call_failed', area='BLING_ENVIO', status='AVISO', details={'error': str(exc)[:220], 'responsible_file': RESPONSIBLE_FILE})
        return False
...
def patch_bling_api_base_urls() -> None:
    ...
    verified_sender_patched = _install_verified_sender()
    if changed_modules or complete_update_patched or review_engine_patched or image_client_patched or verified_sender_patched or not _PATCH_DONE:
        add_audit_event('bling_api_base_runtime_patch_applied', area='BLING_ENVIO', status='OK' if changed_modules or complete_update_patched or review_engine_patched or image_client_patched or verified_sender_patched else 'SEM_ALTERACAO', details={'changed_modules': changed_modules, 'correct_api_base_url': CORRECT_API_BASE_URL, 'complete_product_update_patched': complete_update_patched, 'review_engine_patched': review_engine_patched, 'image_client_patched': image_client_patched, 'verified_sender_patched': verified_sender_patched, 'create_after_stale_404_enabled': True, 'smart_diff_alias_patch': True, 'api_rebuild_client': 'bling_app_zero/core/bling_v3_product_client.py', 'verified_sender': 'bling_app_zero/core/verified_api_sender.py', 'responsible_file': RESPONSIBLE_FILE})
    _PATCH_DONE = True
