...
def _normalizar_preco(valor: Any) -> str:
    if isinstance(valor, (int, float)):
        numero = float(valor)
        return f"{numero:.2f}".replace(".", ",") if numero > 0 else ""
...
def _normalizar_gtin(valor: Any) -> str:
    ...
    return numeros if len(numeros) in {8, 12, 13, 14} else ""
...
def _extrair_jsonld(soup: BeautifulSoup) -> Dict[str, Any]:
    for script in soup.find_all("script", attrs={"type": "application/ld+json"}):
        data = _json_loads_seguro(script.string or script.get_text(" ", strip=True))
        if not data:
            continue
...
def _extrair_estoque_jsonld(jsonld: Dict[str, Any]) -> int:
    ...
    if isinstance(offers, dict):
        return _normalizar_estoque(offers.get("availability"))
...
def extract_product_from_page(html, url):
    ...
    marca = _safe_str(brand.get("name")) if isinstance(brand, dict) else _safe_str(brand)
