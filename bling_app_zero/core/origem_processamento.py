...
def processar_upload_xml(arquivo: Any) -> tuple[pd.DataFrame | None, dict]:
    nome = nome_arquivo(arquivo)
    hash_ref = hash_arquivo_upload(arquivo)
...
def extrair_texto_pdf(arquivo: Any) -> str:
    try:
        arquivo.seek(0)
...
def processar_upload_pdf(arquivo: Any) -> tuple[pd.DataFrame | None, dict]:
    nome = nome_arquivo(arquivo)
    hash_ref = hash_arquivo_upload(arquivo)
...
def processar_upload_arquivo_unificado(uploaded_file: Any) -> tuple[pd.DataFrame | None, dict]:
    if uploaded_file is None:
        return None, {
            "tipo": "",
            "nome": "",
