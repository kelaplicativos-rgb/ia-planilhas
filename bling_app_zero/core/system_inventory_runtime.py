...
RUNTIME_EXTRA_INVENTORY: tuple[SystemInventoryItem, ...] = (
    ...
    SystemInventoryItem(
        id='mirror_site_reader',
        name='Leitura monitorada de produtos do site',
        status=STATUS_ATIVO,
        layer='JOB',
        purpose='Lê os produtos encontrados pelo ciclo agendado usando o pipeline de site e devolve dados estruturados para planejamento.',
        owner_spine='Mirror Discovery Cycle / Site Pipeline',
        main_paths=('bling_app_zero/core/bling_mirror_extract.py',),
        risk='Ainda não altera dados no Bling. Pode consumir tempo de rede conforme o site e o limite configurado.',
    ...
    SystemInventoryItem(
        ...
        status=STATUS_ATIVO,
        layer='JOB',
        purpose='Executa a etapa monitorada do agendador: varredura controlada do site, leitura de produtos, plano de decisões e log persistente.',
        owner_spine='Mirror Store / Executor',
        main_paths=('bling_app_zero/core/bling_mirror_cycle.py',),
        risk='Ainda não compara contra o Bling e não aplica alterações; apenas lê dados e gera plano monitorado.',
        action='Próximo passo: conectar comparação com Bling e envio somente de diferenças.',
    ),
    SystemInventoryItem(
        ...
        status=STATUS_ATIVO,
        layer='JOB',
        purpose='Permite execução por Cron/Worker fora da tela, lendo a configuração persistente, rodando ciclo monitorado e registrando status sem loop no Streamlit.',
        owner_spine='Mirror Store / Render Cron',
        main_paths=('bling_app_zero/jobs/mirror_executor.py',),
        risk='Aplicação real por API ainda está bloqueada até ligar comparação com Bling e logs transacionais.',
        action='Próximo passo: ligar comparação + envio somente de diferenças.',
    ),
)
