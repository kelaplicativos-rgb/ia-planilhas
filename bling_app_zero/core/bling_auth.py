...
class BlingAuthManager:
    ...
    def build_authorize_url(self, force_reauth: bool = False) -> Optional[str]:
        ...
        redirect_uri = f"{redirect_uri}{sep}bi={self.user_key}"
    ...
    def exchange_code_for_token(self, code: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if resp.status_code >= 400:
                return False, f"Erro OAuth: {resp.status_code} | {payload}"
            ...
            return False, "Timeout ao autenticar com Bling."
        except Exception as exc:
            return False, f"Erro OAuth: {exc}"
    ...
    def refresh_access_token(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if resp.status_code >= 400:
                return False, f"Erro refresh: {resp.status_code} | {payload}"
    ...
    def _hydrate_company_name_from_jwt(self) -> None:
        ...
        for key in (
            ...
            if isinstance(val, str) and val.strip():
                self.store.update_company_name(val.strip(), user_key=self.user_key)
                return
