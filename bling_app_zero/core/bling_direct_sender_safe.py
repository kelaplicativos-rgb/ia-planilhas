...
PRODUCT_RESOLUTION_CACHE_KEY = 'bling_safe_product_resolution_cache_v8'
CATEGORY_RESOLUTION_CACHE_KEY = 'bling_safe_category_resolution_cache_v4'
...
COLUMN_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    'marca': ('marca', 'fabricante'), 'unidade': ('unidade', 'un'), 'ncm': ('ncm',),
...
def _session_cache(key: str) -> dict[str, str]:
    ...
    if not isinstance(cache, dict):
        cache = {}; st.session_state[key] = cache
    return cache
...
def _resolve_product_by_candidate(token: dict[str, Any], candidate: str) -> str:
    ...
    cache = _session_cache(PRODUCT_RESOLUTION_CACHE_KEY); key = candidate.lower()
    if key in cache: return str(cache.get(key) or '')
    ...
    for params in ({'codigo': candidate}, {'criterio': candidate}, {'pesquisa': candidate}):
        try:
            ...
            for item in items:
                ...
                if item_id and (candidate.lower() in [i.lower() for i in _item_identifiers(item)] or len(items) == 1):
                    cache[key] = item_id
...
def _category_name_from_row(row: pd.Series, mapping: dict[str, str]) -> str:
    ...
    parts = [p for p in parts if _norm(p) not in {'home', 'inicio', 'loja', 'produtos', 'produto'}]
...
def _category_response_id(data: Any) -> str:
    if isinstance(data, dict):
        ...
        for key in ('data', 'dados', 'categoria', 'result'):
            value = data.get(key)
            nested = _category_response_id(value)
            if nested: return nested
...
def _resolve_category_id(token: dict[str, Any], category_name: str, *, line: int | None = None, product_code: str = '') -> str:
    ...
    if not name: return ''
    cache = _session_cache(CATEGORY_RESOLUTION_CACHE_KEY); key = name.lower()
    if key in cache: return str(cache.get(key) or '')
    headers = _headers(token); lookup_errors: list[str] = []
        ...
        for params in ({'descricao': name}, {'nome': name}, {'criterio': name}, {'pesquisa': name}):
            try:
                ...
                if response.status_code >= 400:
                    lookup_errors.append(f'{path} GET {response.status_code}'); continue
    ...
    for path in _category_paths():
        for payload in ({'descricao': name}, {'nome': name}, {'descricao': name, 'tipo': 'P'}):
            try:
                ...
                if response.status_code >= 400:
                    create_errors.append(f'{path} POST {response.status_code}: {str(response.text or "")[:180]}'); continue
                data = response.json() if str(response.text or '').strip() else {}
                item_id = _category_response_id(data)
                if item_id:
                    cache[key] = item_id
...
def _valid_image_urls(raw: str) -> list[str]:
    ...
    for piece in re.split(r'[|,;\n]+', str(raw or '')):
        ...
        if '@' in url.rsplit('/', 1)[-1] and not re.search(r'\.(png|jpg|jpeg|webp)(\?|$)', url.lower()): continue
...
def _load_stock_deposits(token: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if isinstance(cached, list) and cached: return [i for i in cached if isinstance(i, dict)]
    deposits: list[dict[str, str]] = []; errors: list[str] = []
    for path in _unique_non_empty([_secret('stock_deposits_path', ''), '/estoques/depositos', '/depositos', '/estoque/depositos']):
        try:
            ...
            if response.status_code >= 400:
                errors.append(f'{path}: HTTP {response.status_code}'); continue
            for item in _extract_items(response.json()):
                nested = item.get('deposito') if isinstance(item.get('deposito'), dict) else {}
                dep_id = str(item.get('id') or item.get('idDeposito') or item.get('id_deposito') or item.get('codigo') or nested.get('id') or nested.get('idDeposito') or '').strip()
                ...
                if dep_id or name: deposits.append({'id': dep_id, 'nome': name})
            if deposits:
...
def _resolve_deposit_id(token: dict[str, Any], preferred_name: str = '') -> str:
    ...
    for item in deposits:
        dep_id, name = str(item.get('id') or '').strip(), str(item.get('nome') or '').strip().lower()
        if dep_id and wanted and (wanted == name or wanted == dep_id.lower()):
            st.session_state[API_STOCK_DEPOSIT_ID_KEY] = dep_id; return dep_id
    if len(deposits) == 1 and str(deposits[0].get('id') or '').strip():
        dep_id = str(deposits[0].get('id') or '').strip(); st.session_state[API_STOCK_DEPOSIT_ID_KEY] = dep_id; return dep_id
    return ''
...
def _stock_endpoint_attempts(product_id: str) -> list[tuple[str, str]]:
    ...
    for method, path in raw:
        ...
        if path and '{' not in path and (method, path) not in out: out.append((method, path))
    return out
...
def _base_cadastro_payload(row: pd.Series, mapping: dict[str, str]) -> dict[str, Any] | None:
    ...
    if descricao and descricao.lower() != nome.lower(): payload['descricaoCurta'] = descricao
    preco = _number_value(_value(row, mapping, 'preco'))
    if preco is not None and preco >= 0: payload['preco'] = _api_number(preco)
    marca = _clean_text(_value(row, mapping, 'marca'), 60)
    if marca and not marca.lower().startswith(('mega center', 'stoqui')): payload['marca'] = marca
    ncm = _digits_only(_value(row, mapping, 'ncm'))
    if len(ncm) == 8: payload['tributacao'] = {'ncm': ncm}
...
def _cadastro_payload_variants(token: dict[str, Any], row: pd.Series, mapping: dict[str, str], *, line: int | None = None) -> list[tuple[str, dict[str, Any]]]:
    ...
    if category_name:
        add_audit_event('bling_safe_category_payload_status', area='BLING_ENVIO', status='OK' if category_id else 'AVISO', details={'line': line, 'category': category_name, 'category_id': category_id, 'category_status': 'CATEGORIA_ID_RESOLVIDA' if category_id else 'CATEGORIA_NAO_RESOLVIDA', 'fallback_description_removed': not bool(category_id), 'responsible_file': RESPONSIBLE_FILE})
    image_urls = _valid_image_urls(_value(row, mapping, 'imagens'))
    full = dict(base); with_category = dict(base)
    if category_payload:
        full['categoria'] = category_payload; with_category['categoria'] = category_payload
    if image_urls: full['midia'] = {'imagens': [{'link': url} for url in image_urls]}
    variants: list[tuple[str, dict[str, Any]]] = []; seen: set[str] = set()
    for label, payload in (('completo_categoria_id_imagem', full), ('categoria_id_sem_imagem', with_category), ('minimo_sem_categoria_imagem', base)):
        cleaned = _clean_payload(payload); marker = repr(sorted(cleaned.items()))
        if cleaned and marker not in seen:
            variants.append((label, cleaned)); seen.add(marker)
    return variants


def _cadastro_preview_payloads(df: pd.DataFrame, *, limit: int = 5) -> list[dict[str, Any]]:
    token, _meta = load_token(); mapping = _column_map(df.columns); previews: list[dict[str, Any]] = []
    for position, (_index, row) in enumerate(df.fillna('').head(limit).iterrows(), start=1):
        if isinstance(token, dict) and token.get('access_token'):
            variants = _cadastro_payload_variants(token, row, mapping, line=position); payload = variants[0][1] if variants else None
            cat = payload.get('categoria', {}) if isinstance(payload, dict) else {}; motivo = 'Categoria API: ID resolvido' if isinstance(cat, dict) and cat.get('id') else 'Categoria API: não resolvida ou ausente'
        else:
            payload = _base_cadastro_payload(row, mapping); motivo = 'Bling não conectado: preview sem resolução de categoria por API.' if payload else 'Nome/código insuficiente para cadastro.'
...
def _stock_preview_payloads(df: pd.DataFrame, *, limit: int = 5) -> list[dict[str, Any]]:
    token, _meta = load_token(); mapping = _column_map(df.columns); previews: list[dict[str, Any]] = []
    if not isinstance(token, dict) or not token.get('access_token'): return [{'payload': {}, 'status': 'IGNORADO', 'motivo': 'Bling não conectado.'}]
    deposit_id = _resolve_deposit_id(token)
    for _index, row in df.fillna('').head(limit).iterrows():
        qty = _number_value(_value(row, mapping, 'quantidade')); product_id = ''
        for candidate in _row_candidates(row, mapping):
            product_id = _resolve_product_by_candidate(token, candidate)
            if product_id: break
        if not product_id: previews.append({'payload': {}, 'status': 'IGNORADO', 'motivo': 'Produto não resolvido no Bling por Código/GTIN/ID.'})
        elif not deposit_id: previews.append({'payload': {}, 'status': 'IGNORADO', 'motivo': 'Depósito não resolvido no Bling.'})
        elif qty is None: previews.append({'payload': {}, 'status': 'IGNORADO', 'motivo': 'Quantidade/saldo inválido.'})
        else: previews.append({'payload': _stock_payload(product_id, deposit_id, qty), 'status': 'OK', 'motivo': ''})
    return previews


def preview_payloads(df: pd.DataFrame, operation: str, *, limit: int = 5) -> list[dict[str, Any]]:
    normalized = normalize_operation(operation)
    if normalized == OP_CADASTRO: return _cadastro_preview_payloads(df, limit=limit)
    if normalized == OP_ESTOQUE: return _stock_preview_payloads(df, limit=limit)
...
def _send_cadastro_dataframe_to_bling(df: pd.DataFrame, *, limit: int | None = None, progress_callback: Callable[[dict[str, Any]], None] | None = None) -> DirectSendResult:
    ...
    rows = df.fillna('').head(limit) if limit else df.fillna(''); mapping = _column_map(rows.columns)
    total = len(rows); sent = failed = skipped = 0; errors: list[str] = []; create_path = _secret('product_create_path', '/produtos') or '/produtos'
    ...
    for position, (index, row) in enumerate(rows.iterrows(), start=1):
        line = int(index) + 1 if isinstance(index, int) else position; variants = _cadastro_payload_variants(token, row, mapping, line=line)
        if not variants:
            skipped += 1
            if len(errors) < 8: errors.append(f'Linha {line}: nome/código insuficiente para cadastro.')
            continue
        ok = False; attempt_logs: list[dict[str, Any]] = []; last_response: requests.Response | None = None
        for strategy, payload in variants:
            try:
                response = requests.post(_url(create_path), headers=_headers(token), json=payload, timeout=SEND_TIMEOUT); last_response = response
                cat = payload.get('categoria') if isinstance(payload.get('categoria'), dict) else {}
                attempt_logs.append({'strategy': strategy, 'status': int(response.status_code), 'category_id': cat.get('id', ''), 'payload_keys': sorted(payload.keys()), 'response_preview': str(response.text or '')[:300]})
                if response.status_code < 400:
                    ok = True; add_audit_event('bling_safe_cadastro_strategy_succeeded', area='BLING_ENVIO', status='OK', details={'line': line, 'strategy': strategy, 'category_id': cat.get('id', ''), 'attempts': attempt_logs[-3:], 'responsible_file': RESPONSIBLE_FILE}); break
        ...
        if ok: sent += 1
            ...
            failed += 1; status = getattr(last_response, 'status_code', 'sem resposta'); preview = str(getattr(last_response, 'text', '') or '')[:500]
            if len(errors) < 8: errors.append(f'Linha {line}: Bling recusou cadastro ({status}) após {len(variants)} tentativa(s). {preview}')
    ...
    add_audit_event('bling_safe_cadastro_send_finished', area='BLING_ENVIO', status='OK' if failed == 0 else 'PARCIAL', details={'attempted': total, 'sent': sent, 'failed': failed, 'skipped': skipped, 'payload_mode': 'category_id_only_no_description_fallback_with_audit', 'responsible_file': RESPONSIBLE_FILE})
...
def _send_stock_dataframe_to_bling(df: pd.DataFrame, *, limit: int | None = None, progress_callback: Callable[[dict[str, Any]], None] | None = None) -> DirectSendResult:
    ...
    if not isinstance(token, dict) or not token.get('access_token'):
        return DirectSendResult(0, 0, 0, len(df) if isinstance(df, pd.DataFrame) else 0, ('Bling não conectado. Conecte o app antes de enviar direto.',))
    rows = df.fillna('').head(limit) if limit else df.fillna(''); mapping = _column_map(rows.columns)
    total = len(rows); sent = failed = skipped = 0; errors: list[str] = []; not_found: list[int] = []
    ...
    for position, (index, row) in enumerate(rows.iterrows(), start=1):
        line = int(index) + 1 if isinstance(index, int) else position; qty = _number_value(_value(row, mapping, 'quantidade'))
        if qty is None:
            skipped += 1
            if len(errors) < 8: errors.append(f'Linha {line}: quantidade/saldo ausente ou inválido.')
            continue
        product_id = ''
        for candidate in _row_candidates(row, mapping):
            product_id = _resolve_product_by_candidate(token, candidate)
            if product_id: break
        if not product_id:
            failed += 1; not_found.append(int(index) if isinstance(index, int) else position - 1)
            if len(errors) < 8: errors.append(f'Linha {line}: produto não encontrado no Bling por Código/SKU/GTIN/ID.')
            continue
        deposit_id = _resolve_deposit_id(token, _value(row, mapping, 'deposito')) or default_deposit_id
        if not deposit_id:
            failed += 1
            if len(errors) < 8: errors.append(f'Linha {line}: depósito não resolvido no Bling.')
            continue
        payload = _stock_payload(product_id, deposit_id, qty); last_response: requests.Response | None = None; ok = False; attempts: list[dict[str, Any]] = []
        for method, path in _stock_endpoint_attempts(product_id):
            try:
                response = requests.request(method, _url(path), headers=_headers(token), json=payload, timeout=SEND_TIMEOUT); last_response = response
                ...
                if response.status_code < 400: ok = True; break
                if response.status_code in {401, 403}: break
            except Exception as exc:
                attempts.append({'method': method, 'path': path, 'status': 'EXCEPTION', 'error': str(exc)[:180]})
        if ok: sent += 1
        else:
            failed += 1; status = getattr(last_response, 'status_code', 'sem resposta'); preview = str(getattr(last_response, 'text', '') or '')[:180]
            if len(errors) < 8: errors.append(f'Linha {line}: Bling recusou estoque ({status}). Primeiro endpoint tentado: /estoques. {preview}')
...
def send_dataframe_to_bling(df: pd.DataFrame, operation: str, *, limit: int | None = None, progress_callback: Callable[[dict[str, Any]], None] | None = None) -> DirectSendResult:
    normalized = normalize_operation(operation)
    if normalized == OP_CADASTRO: return _send_cadastro_dataframe_to_bling(df, limit=limit, progress_callback=progress_callback)
    if normalized == OP_ESTOQUE: return _send_stock_dataframe_to_bling(df, limit=limit, progress_callback=progress_callback)
