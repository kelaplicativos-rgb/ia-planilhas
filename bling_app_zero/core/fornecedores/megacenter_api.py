...
TIMEOUT = 20
BASE_URL = "https://megacentereletronicos.com.br/"
...
def _urls_seed_megacenter(categoria: str = "") -> list[str]:
    categoria = _normalizar_texto(categoria)

    seeds_fixas = [
        BASE_URL,
        urljoin(BASE_URL, "categoria/47951-cabos"),
        urljoin(BASE_URL, "categoria/47541-controle-para-televisao"),
        urljoin(BASE_URL, "categoria/47537-caixa-de-som"),
        urljoin(BASE_URL, "categoria/47535-carregadores"),
        urljoin(BASE_URL, "categoria/47533-smartwatch"),
        urljoin(BASE_URL, "categoria/47534-camera"),
    ]

    if categoria:
        slug = categoria.replace(" ", "-")
        seeds_fixas.insert(0, urljoin(BASE_URL, f"categoria/{slug}"))
...
def _extrair_nome(soup: BeautifulSoup, jsonlds: list[dict]) -> str:
    ...
    return _buscar_primeiro_texto(
        soup,
        ["h1", ".product-title", ".product-name", ".nome-produto", '[itemprop="name"]'],
    )
...
def _extrair_descricao(soup: BeautifulSoup, jsonlds: list[dict]) -> str:
    ...
    return _buscar_primeiro_texto(
        soup,
        [".product-description", ".descricao", ".description", '[itemprop="description"]'],
    )
...
def _extrair_preco(soup: BeautifulSoup, jsonlds: list[dict]) -> str:
    ...
    texto_preco = _buscar_primeiro_texto(
        soup,
        ["[itemprop='price']", ".price", ".preco", ".product-price", ".valor"],
    )
    if texto_preco:
...
def _extrair_codigo(soup: BeautifulSoup, jsonlds: list[dict], url_produto: str) -> str:
    for item in jsonlds:
        for chave in ["sku", "productID", "mpn", "gtin13", "gtin"]:
...
def _extrair_categoria(soup: BeautifulSoup) -> str:
    for seletor in [".breadcrumb a", ".breadcrumbs a", '[aria-label="breadcrumb"] a']:
...
def _extrair_produto_megacenter(url_produto: str) -> dict:
    try:
...
def buscar_produtos_mega_center(
    fornecedor: str = "mega_center",
    categoria: str = "",
    operacao: str = "",
    config: dict | None = None,
) -> pd.DataFrame:
    _ = fornecedor, operacao

    if isinstance(config, dict):
        categoria = _safe_str(config.get("categoria")) or categoria

    seeds = _urls_seed_megacenter(categoria=categoria)
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        futuros = {executor.submit(_extrair_produto_megacenter, link): link for link in links}
