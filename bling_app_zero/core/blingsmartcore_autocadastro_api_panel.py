...
from bling_app_zero.core.bling_autocadastro_api import autocadastrar_e_atualizar_estoque
...
def _render_result() -> None:
    ...
    if attempted == 0:
        ...
        st.success(f'AutoCadastro concluído: {created}/{attempted} produto(s) cadastrado(s) e {stock_updated} estoque(s) atualizado(s).')
    elif created > 0:
        st.warning(f'AutoCadastro parcial: {created}/{attempted} cadastrado(s), {stock_updated} estoque(s) atualizado(s), {failed} falha(s), {skipped} ignorado(s).')
    else:
        st.error(f'AutoCadastro não concluiu: 0/{attempted} cadastrado(s), {failed} falha(s), {skipped} ignorado(s).')
    cols = st.columns(4)
    cols[0].metric('Processados', attempted)
    cols[1].metric('Cadastrados', created)
...
def render_autocadastro_panel(download_df: pd.DataFrame, result_payload: dict[str, Any], *, key: str) -> None:
    ...
    st.warning(f'{len(df_not_sent)} produto(s) não foram confirmados no envio. {len(eligible)} elegível(is) para cadastro direto via API.')
    st.download_button(
    ...
    if not eligible.empty:
        if st.button('Cadastrar no Bling e atualizar estoque automaticamente', use_container_width=True, key=f'autocadastro_api_run_{key}_{len(eligible)}'):
            progress_bar = st.progress(0, text='AutoCadastro via API...')
            status_box = st.empty()
            ...
            def _progress(payload: dict[str, Any]) -> None:
                ...
                text = f'AutoCadastro: {processed}/{total} · cadastrados {created} · estoque {stock_updated} · falhas {failed}'
                progress_bar.progress(min(100, int(ratio * 100)), text=text)
                status_box.caption(text)

            result = autocadastrar_e_atualizar_estoque(eligible, progress_callback=_progress)
            ...
            try:
                ...
                pass
            add_audit_event('blingsmartcore_autocadastro_api_panel_finished', area='AUTOCADASTRO', status='OK' if result.failed == 0 else 'PARCIAL', details={'attempted': result.attempted, 'created': result.created, 'stock_updated': result.stock_updated, 'failed': result.failed, 'responsible_file': RESPONSIBLE_FILE})
            st.rerun()
